...
(function ($) {
  ...
  $(window).scroll(function(){
    if($(window).scrollTop() > 100){
      $('header.mainheader').addClass('inner shadow-sm');
      }
     else{
      $('header.mainheader').removeClass('inner shadow-sm');
     }
});
$("#TableOfContents ul li a").click(function(event) {
    event.preventDefault();
    var tr = $(this).attr('href');
    $('html, body').animate({
        scrollTop: $(tr).offset().top-$('header').height()-20
    }, 500);
});
})(jQuery);
